 ...
 * @author O. J. Coleman
 */
public class ViewTableSeriesParametrised extends TableViewSeries implements Parametrised {
	ListSet<Parameter<?>> params;
	Set<ParameterListener> changeListeners;
	
	public ViewTableSeriesParametrised(DataTable source) {
		super(source);
	...
	public synchronized void dataChanged(DataEvent event) {
		...
		if (!types.isEmpty()) {
			...
			fireParameterChange(new ParameterEvent(ViewTableSeriesParametrised.this, types.toArray(ta)));
		}
	}
	...
	private class Param extends Parameter<Boolean> {
		...
		public Boolean getValue() {
			return ViewTableSeriesParametrised.this.hasSeries(label);
		}
