...
public abstract class ViewTable extends AbstractDataTable implements DataListener {
	 ...
	 */
	protected final List<DataTable> source;
	...
	public ViewTable(DataTable... inputTables) {
		...
		if (inputTables == null) {
			...
			source = Arrays.asList(inputTables);
		}
		series = new LSListMap<>();
		for (DataTable s : source) {
	...
	public void dataChanged(DataEvent event) {
		if (source.contains(event.affected)) {
	...
	private void updateSeriesWrapper(List<Object> eventTypes) {
		ListMap<String, DataSeries<?>> origSeries = new LSListMap<>(series);
