...
void setup() {
  // DataSeries represent a sequence (or vector or list) of data items. 
  // The data represented by a DataSeries may be numeric, strings, 
  // dates/times or any other type of object.
  // Numeric DataSeries have numerous built-in methods for performing 
  // arithmetic operations over the data. For the complete API see 
  // https://olivercoleman.github.io/hivis/reference/hivis/data/DataSeries.html
  
  // (Advanced Java developers: DataSeries have a generic type parameter 
  // for the data they represent, however this can generally be safely 
  // ignored. This is done here for readability and simplicity (one of 
  // the core design principles of HiVis). The examples below indicate 
  // when type must be taken into consideration.)
  ...
  // Note: because we haven't specified the data type represented by the 
  // series (eg DataSeries<Integer>) we use the asInt() method to get a 
  // view of the intNumbers series as explicitly representing integers, 
  // so the type of the variable 'v' above can be int.
  
  println("\nintNumbers =>\n\t" + intNumbers);
  println("\nrealNumbers =>\n\t" + realNumbers);
  
  
  // We can set the values in a series - providing it is not calculated 
  // from another data set, more on this below. 
  ...
  // Some thing to note here is that we assigned a real (double) number 
  // value to a series that stores integer (int) values. In general 
  // HiVis will do its best to accommodate the interchange of data of 
  // different types when it makes sense to do so. (If we tried to set a 
  // value in a series representing integers to, say 1.5, then an error 
  // would occur.)
  
  
  // We can derive new series by performing simple arithmetic operations 
  // on each element of an existing series:
  DataSeries plus1 = realNumbers.add(1);
  println("\nplus1 = realNumbers.add(1) =>\n\t" + plus1);
  // Similar functions exist for subtract, multiply and divide.
  // (Advanced Java developers: the number type of the new series will 
  // be set to accommodate the calculated values. For example if one 
  // series stores ints and the other floats then the new series will 
  // be of type double. The divide operation always returns a series 
  ...
  // We can also derive a series from another by performing common 
  // numeric operations such as exponential, logarithm, square root, 
  // trigonometric and many others on each element of the series.
  ...
  // Note that there is also toRange(min, max) to allow specifying the 
  // range to scale the values to. 
  
  // sort(), which creates a series containing the values in the 
  // original series sorted according to their "natural" ordering:
  ...
  // We can derive new series by applying a custom function to each 
  // element of a series. You define the function via an implementation 
  // of the Function class. The type of the argument for the apply 
  // method should match the type of the data represented by the series.
  ...
  println("\ncustomFunc = plus1^3 =>\n\t" + customFunc);
  // (Advanced Java developers: the return type of the apply method 
  // should be Object unless you've specified, via generics, the input 
  // and output type of the function, for example:
  // new Function<MyInputType, MyOutputType>()...)

  // A function doesn't have to be numeric, for the input or the return 
  // value, nor does the input and output type have to be the same.
  ...
  // The series created above, starting with 'plus1', are "Views" of the 
  // original series but with the operation performed on each element, so 
  // changes in the original series will be reflected in the new series.
  
  // NOTE: These DataSeries that are calculated from other data sets 
  // (DataSeries or otherwise) cannot have their values directly 
  // manipulated via the set() method.  
  
  // Also note that the customFunc series is a View of the plus1 series, 
  // which in turn is a View of realNumbers; changes to the underlying 
  // data will "bubble up" through the chain of views.
  ...
  // We can create a series that provides a view of the elements in a 
  // series filtered and/or rearranged:
  ...
  // (Advanced Java developers: DataValues also have a generic type, 
  // but again this can usually be safely ignored.)
  ...
  // If the series changes then the value represented by the DataValue 
  ...
  // (Advanced Java developers: you may have noticed that we are calling 
  // intNumbers.min() twice, however the minimum is not actually 
  // calculated twice as the same DataValue "view" is cached and reused 
  // by subsequent calls to the min() method. The same is true for the 
  // other statistical methods - and even within the methods where 
  // possible, eg mean() will create a cache of the sum DataValue as 
  // well as the mean DataValue).
  
  // Changes in the min and max DataValues (which reflect changes in the 
  // intNumbers series) will be reflected in the calculated series:
  intNumbers.set(0, 0);
  println("\nintNumbers (changed first value) =>\n" + intNumbers);
  println("\nintNumbersUnitRange reflecting changed value in intNumbers =>\n" + intNumbersUnitRange);
  
  
  // We can also create a DataValue from one or more Series with a 
  // custom function:
  ...
  // Again, the dotProduct DataValue is a view, of the two input 
  // DataSeries, so changes to either vector1 or vector2 will be 
  // reflected in it.
  ...
  // We can obtain sorted views over a series (the sorted view will be 
  // updated when the source series changes).
  ...
  // We can obtain "grouped" views over a series. A grouping over a 
  // series is represented as a DataMap (which represents a mapping from 
  // keys to values, see the Maps example). The values of the DataMap 
  // are DataSeries containing the values in that group. We can group by 
  // the values' own equality, in which case the key for each group is a 
  // value such that key.equals(v) for all values in the group:
  DataMap randomIntsGrouped = randomInts.group();
  println("\nrandomIntsGrouped = randomInts.group() => " + randomIntsGrouped);
  // Or we can group using a custom "key function" (the same kind of 
  // function used in the apply() method previously), in which case 
  // the key for each group is a value such that 
  // key.equals(keyFunction(v)) for all values in the group:
  ...
  // The output of the key function can be any kind of value (we used 
  // strings above, but could just have easily returned the raw value 
  // given by "value % 2"). The ordering of the values in the groups 
  // matches the ordering of those values in the original series.
